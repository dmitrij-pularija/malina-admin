...
const RatingWaitersList = ({userData, users, waiters, stores, t}) => {
  // ** Store Vars
  ...
  const [currentWaiter, setCurrentWaiter] = useState({ value: '', label: t('selectWaiter') })
  ...
  waitersOptions.unshift({ value: '', label: t('showAll') })
    ...
    <Fragment>
      <Card>
        <CardBody>
          <Row>
            <Col md='4'>
              <Label for='role-select'>{t('waiter')}</Label>
              <Select
                isClearable={false}
                value={currentWaiter}
